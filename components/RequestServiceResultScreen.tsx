...
interface RequestServiceResultScreenProps {
  setScreen: (screen: Screen) => void;
}

const RequestServiceResultScreen: React.FC<RequestServiceResultScreenProps> = ({ setScreen }) => {
  const goBack = () => {
    setScreen(Screen.RequestService);
  };
  ...
  const renderHeader = (title: string) => (
     <header className="relative bg-white shadow-sm p-4 flex justify-between items-center sticky top-0 z-20 shrink-0 h-20">
        <button onClick={() => setScreen(Screen.MerchantServices)} className="w-10 h-10 flex items-center justify-center bg-gray-100 rounded-[5px] text-gray-600 hover:text-primary hover:bg-primary-light transition-colors" aria-label="Back">
...
export default RequestServiceResultScreen;
